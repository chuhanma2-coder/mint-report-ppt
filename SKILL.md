...
2. Classify every source unit as `required-visible`, `supporting-visible`, or `traceability`. Group all facts by the task-card `outlineItem` before planning pages. First try to place one complete outline item on one readable page. Split it only when the measured readable capacity fails, and only at a named natural evidence/business boundary. Never begin from “one topic/question = one slide”. Write an explicit transition for every content slide after the first; the title chain alone must read as one management story.
3. Build a Page Evidence Bundle for every content slide, then write light Slide IR conforming to `schemas/slide-ir.schema.json`. Every required/supporting unit needs a `visibleFacts` entry whose text is actually present in that module's visible title, text, metric, chart data, or table data; an `evidenceRef` alone is not content. AI writes facts, semantic intent, evidence hierarchy, story cluster, and decision unit—not `pageComposition`, coordinates, fonts, colors, or PowerPoint code.
...
- Outline integrity: one task-card outline item is the first page-planning unit. Multiple pages for one outline item require sequential `outlinePart` values and, from part 2 onward, `outlineSplit.reason`, `naturalBoundary`, and `continuationOf`. The deterministic gate rejects adjacent parts that still fit one readable page estimate and rejects unnecessary fragmentation.
- Visual hierarchy: every content page has one P0 written conclusion, headline fact/metric, or source image; one or two P1 supporting objects; and subordinate P2 context. Charts, diagrams, and tables are evidence carriers, not automatic page-dominant objects. A chart is P1 by default and must be accompanied by at least two substantive non-chart carriers that explain the fact and management meaning. Image-led pages may use a dominant source image with a narrow vertically stacked support rail.
