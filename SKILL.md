...
- Outline integrity: one task-card outline item is the first page-planning unit. Multiple pages for one outline item require sequential `outlinePart` values and, from part 2 onward, `outlineSplit.reason`, `naturalBoundary`, and `continuationOf`. Browser-measured minimum-page partitioning—not unit scores or independent-question labels—determines whether adjacent parts can merge.
