...
Before planning, build a Canonical Source Ledger from the selected raw inputs. Complete the reviewed design-director brief before authoring IR; keep both stages in the current Agent workflow. Read `rc5-planning.md` for grounding and `design-intent.md` for the executable brief. A page needs a supported claim and sufficient visible evidence, with no minimum carrier count. Outline identity is not a forced page boundary. Explicit single-page instructions block rather than silently split when capacity fails.
