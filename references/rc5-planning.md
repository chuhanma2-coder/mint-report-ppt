...
`preflightMode` defaults auto. Preview: create/review the brief, run `node scripts/preflight.mjs brief.json canonical-source-ledger.json`, return its readable explanation and questions; no PPT. Off suppresses interaction, not grounding or design validation. The local design development contract additionally requires `designBriefs` and a hash-bound `designBriefReview`; historical IR must be reviewed against raw inputs before this build, not auto-stamped compatible. See `design-intent.md` for the design-then-authoring workflow.
