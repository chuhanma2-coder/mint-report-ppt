...
Retain the complete original fact in the source ledger, not automatically as a visible paragraph. Author human-facing `displayCopy` under `presentation-copy.md`. A paraphrase needs `requiredComponents`, an array of strings or `{role,text,alternatives}` objects covering its complete meaning, and `componentReview: {status: "reviewed", sourceText: <unchanged original text>}`. This is a recorded semantic-review action, not an automatic claim by the matcher. Do not approve components without comparing against the source. Numeric and key qualifier checks run in addition to this review. Explicitly reviewed `terminology: [{source,display,reason:"translation"|"name-normalization"}]` may replace English terms, never numbers or Chinese forecast/currency/condition qualifiers. Record complete meaning rather than selecting a convenient shorter substring.
