...
import { requiredHtmlCore, skillVersion, theme } from "./config.mjs";
...
export function createTaskCard(input) {
  ...
  const card = {
    ...
    skillVersion,
    htmlCoreVersion: requiredHtmlCore,
    themeVersion: theme.themeVersion,
    pptMasterVersion: "mint-fresh-master/1",
    ...
    sections,
    designContract: { ...theme.slide, fonts: theme.fonts, palette: theme.palette, chartSeries: theme.chartSeries },
    publication: { collaboration: "section-pptx", primary: "html", finalPptx: "optional", pdf: "optional-native-export" }
  };
...
export function readTaskCard(file) {
  ...
  if (card.skillVersion !== skillVersion || card.themeVersion !== theme.themeVersion || card.htmlCoreVersion !== requiredHtmlCore) throw new Error(`Task card requires mint-report-ppt ${card.skillVersion}, theme ${card.themeVersion}, html core ${card.htmlCoreVersion}; installed versions differ`);
